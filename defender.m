function [ball, players] = defender(ball,players,id)
    ...
    if players(id).lastKick == 1       % if ball held
        if distToGoal < goalThresh
            if distToBall > tolerance       % if dis tance to ball > torelance -> move to ball
                [player_x,player_y,player_speed,player_acceleration]=PD_Controller(player_x,player_y,player_speed,kp,kd,distance_to_ball,prev_distance_to_ball);
            else
    ...
    elseif players(teamates(1)).lastKick == 1 || players(teamates(2)).lastKick == 1 || players(teamates(3)).lastKick == 1
        ...
        if ballDistToOwnGoal > 3
            if distToBall > tolerance       % if distance to ball > torelance -> move to ball
                desire_position = (ball.pos + ownGoalPosition) / 2;      % block goal
                distance_to_dpos = [player_x - desire_position(1), player_y - desire_position(2)];
                [player_x,player_y,player_speed,player_acceleration]=PD_Controller(player_x,player_y,player_speed,kp,kd,distance_to_dpos,prev_distance_to_ball);
            else
